...
[the roadmap](roadmap.md#roadmap) for the same four lanes applied to the product phases.

| Lane       | Means                                                                 | Why it's tagged this way                                                                                                                                                                                                                                                              |
| ---------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `[CLOUD]`  | A Claude Code web session, start to finish                            | The default. Measured 2026-08-25: `npm ci` (21s), `npm run typecheck`, `npm run lint`, `npx next build`, and any test that only reads source all run clean. As of 2026-09-03, so does the full DB-backed suite — see [3.7](#37-postgres-without-docker-in-a-cloud-session)            |
| `[LOCAL]`  | Claude, run from a machine that holds something a cloud session can't | Docker specifically — `docker compose up`, the session-start hook's Docker branch — or a credential that only exists on that machine, e.g. Conner's production database connection string. A task that only needs the production DB (a migration, a query) is `[LOCAL]`, not `[NOAH]` |
| `[MANUAL]` | A human, either of you, by hand                                       | What Claude genuinely cannot do at all, in the cloud or locally — clicking, reading, judging, no special account needed                                                                                                                                                               |
| `[NOAH]`   | Noah specifically                                                     | An account or credential **only Noah holds**: GitHub repo admin (Actions secrets, branch protection), the Vercel project dashboard (env vars, deploys, domains), a DNS registrar, or a paid signup needing his payment or identity. No agent has these credentials, and none should   |
...
- **Phase 6's cloud half, and the phase 8 and 9 designs, are on `main` as of 2026-09-04.** One
  ...
  added two rows to the [Outstanding table](#outstanding): applying the `job_runs` migration
  (row 10, `[LOCAL]` — it needs only the production database connection string, which Conner
  holds) and setting `ALERT_WEBHOOK_URL` and the Sentry DSNs (row 11, `[NOAH]` — Vercel dashboard
  and a Sentry signup). Phase 6's own DB-backed tests turned out not to need a desktop at all —
  see [Done row 16](#done) — so nothing from this merge stayed `[LOCAL]` for want of Docker.
  **The rule this document keeps repeating applies here too:** the counts and statuses above were
  re-verified on 2026-09-04 rather than left to update themselves.
...
- **Five milestones**, one per roadmap phase, named to match
  [the roadmap](roadmap.md#roadmap) rows 5-9.
