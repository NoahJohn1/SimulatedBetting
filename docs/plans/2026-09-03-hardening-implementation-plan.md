...
Read it before Task 1. Decisions [D69](../decisions.md#d69--rate-limiting-is-a-postgres-fixed-window-counter-enforced-at-the-action-boundary)
through [D73](../decisions.md#d73--the-smoke-checklist-ships-unvalidated-with-a-run-log) are already
recorded.
...
- **The limiter fails open.** No code path added by this plan may let a limiter failure prevent a
  ...
  ([D70](../decisions.md#d70--the-rate-limiter-fails-open-and-counts-attempts-not-successes)).
...
Admins are limited, not exempt (D69): the failure this guards against — a component re-rendering in
...
Create `src/app/rules/page.tsx`. Root level, outside `(app)`, **no session required** (D72) — a
