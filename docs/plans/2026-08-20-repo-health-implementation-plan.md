...
**Architecture:** Three lanes. Lane H is five browser-only steps that only you can do. Lane C is three tasks a cloud session finishes outright. Lane L is four tasks whose files can be written anywhere but whose gate is the test suite, which needs Docker. All seven coding tasks have **disjoint file ownership** and can run in parallel — except Task 7, which is serial and runs last, because it is the only one that edits the documentation every other task reports into.

**Tech Stack:** GitHub Actions, Claude Code hooks and skills, Vitest, Node 22, Docker Compose, Drizzle.

## The three lanes

Every task belongs to exactly one lane, decided by **who or what can finish it** — not by who
can write the files. A Claude Code cloud session can author every file in this plan. It cannot
run the test suite, because there is no Docker daemon there (`/var/run/docker.sock` does not
...
it passes in 324ms with no database.

**Lane L in a cloud session.** The files can all be written there. The rule for whoever does it:
**leave the suite unrun and say so.** Do not substitute `npm run typecheck && npm run lint` and
...
## Lane H — do these first (browser only)

**Nothing here needs your laptop.** These are GitHub settings — any browser, phone included.
Do them before dispatching any other lane, because Lane C's issue template references labels
that must already exist, and because branch protection is what makes every later commit land
through a real gate.

### H1 — Branch protection on `main`

**The highest-value item in this entire plan.** CI is advisory until it exists: a pull request
with red checks can be merged today.
...
Leave **"Require approvals" off**. With two developers it means you cannot merge your own work
while the other is asleep, and the realistic outcome is that you disable the whole rule.

### H2 — Create the labels
...
### H3 — Create the milestones

Issues → Milestones → New milestone, five of them, named to match
...
Leave due dates empty. A date invented now is a date you will move.

### H4 — Do not create issues from the roadmap yet
...
### H5 — Run the suite before merging Lane L

Lane L's tasks cannot close their own gate in the cloud. Before merging that work:
...
Expected: typecheck clean, 0 lint errors, 547 tests passing, `Compiled successfully`.
...
**Lane H is not coding work** — it is the browser checklist above, and it comes first.

---

# Lane C — a cloud session finishes these completely

Markdown and YAML only. No database, no build, nothing deferred.

---

## Task 3 · LANE C · `decision-log` skill
...
## Task 4 · LANE C · `money-invariants` skill
...
## Task 5 · LANE C · Bug issue template
...
# Lane L — files anywhere, verification needs Docker

Each of these ends in `npm run verify`, which a cloud session cannot run. Write them
wherever is convenient; close them where Postgres exists. See [H5](#h5--run-the-suite-before-merging-lane-l).

---

## Task 1 · LANE L · Guard test for the ledger funnel
...
## Task 2 · LANE L · SessionStart hook
...
## Task 6 · LANE L · CI gaps and Node pinning
...
## Task 7 · LANE L · Documentation — run last, alone
...
`db-migration` skill, at minimum). Write what is actually true after the other tasks landed — check, do not assume.
